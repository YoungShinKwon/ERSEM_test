...
!BOC
      ...
      call self%get_parameter(self%si_remin,'si_remin','',          'silicate remineralization', default=.false.)
      ...
      if (self%si_remin) call self%register_state_dependency(self%id_N5s,'N5s','mmol Si/m^3','silicate')    !YSK
...
!..Set diagnostics
   ...
   !!YSK: Added the remineraliation of silicate on 20200909
      ...
      real(rk) :: R1pP,R1nP
      real(rk) :: N7fP,N5sP
      real(rk) :: fR1N1p,fR1NIn,sugB1
      real(rk) :: RPfP(self%nRP),fRPN7f(self%nRP),n7fsink
      real(rk) :: RPsP(self%nRP),fRPN5s(self%nRP)
      ...
      _LOOP_BEGIN_
         ...
         if (use_iron) then
            ...
            do iRP=1,self%nRP
               !if (fRPn7f(iRP)/=0.0_rk) _SET_ODE_(self%id_RPf(iRP),-fRPn7f(iRP))   !Original
               if (fRPn7f(iRP)/=0.0_rk) _SET_ODE_(self%id_RPf(iRP),-fRPn7f(iRP)+n7fsink)   !YSK on 20200908 to make same with the old version ERSEM
         ...
         if (self%si_remin) then
	   !YSK:  remineralization of silicate 
	   do iRP=1, self%nRP
    	      _GET_(self%id_RPs(iRP), RPsP(iRP))
	   end do 
           fRPN5s=self%sRPr1*RPsP*2
	   do iRP=1,self%nRP
