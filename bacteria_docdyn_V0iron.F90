...
!BOC
      ...
      call self%get_parameter(self%dp,   'dp',   'adim',    'threshop DOC:POC ratio for iron adsorption itnto particles ')
...
!..Set diagnostics
   ...
   subroutine remineralization(self,_ARGUMENTS_DO_)
      ...
      integer :: iRP
      real(rk) :: R1pP,R1nP,R1cP,R2cP,R3cP,docpoc,tdoc,POCf
      real(rk) :: N7fP
      real(rk) :: fR1N1p,fR1NIn
      real(rk) :: RPfP(self%nRP),RPcP(self%nRP),fRPN7f(self%nRP),n7fsink
      ...
      _LOOP_BEGIN_
         ...
         if (use_iron) then
            ...
            _GET_(self%id_R2c,R2cP)
             tdoc=R1cP+R2cP+R3cP
             docpoc=tdoc/sum(RPcP)
            ! n7fsink=self%fsinkX*max(0._rk,N7fP-0.6_rk)

            ! the adsorption into particles is assumed to be regulated by the ratio between DOC and POC relative to a reference ratio
            ! representing the DOC:POC ratio above which iron is stabilised (i.e. no sink). Adsorbed Iron is distributed among the
            ! different type of particles (R4, R6 etc) depending on their relative abundance (Luca 5/2017)) 
            n7fsink=self%fsinkX*max(0._rk,(1._rk-docpoc/self%dp))*N7fP
            ! the following loop accounts for the total amount of carbon in particles which contain iron
            POCf=0._rk
            do iRP=1,self%nRP
               if (RPfP(iRP)/=0.0_rk) POCf=POCf+RPcP(iRP)
            end do

            do iRP=1,self%nRP
            !   if (fRPn7f(iRP)/=0.0_rk) _SET_ODE_(self%id_RPf(iRP),-fRPn7f(iRP))
               if (fRPn7f(iRP)/=0.0_rk) _SET_ODE_(self%id_RPf(iRP),-fRPn7f(iRP)+n7fsink*RPcP(iRP)/POCf)
            end do
