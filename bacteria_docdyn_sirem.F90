...
module ersem_bacteria_docdyn_sirem
   ...
   type,extends(type_ersem_pelagic_base),public ::  type_ersem_bacteria_docdyn_sirem
      ! Variables
      ...
      type (type_state_variable_id) :: id_N1p,id_N4n,id_N7f,id_N5s
      type (type_dependency_id)     :: id_ETW,id_eO2mO2
      type (type_state_variable_id),allocatable,dimension(:) :: id_RPc,id_RPp,id_RPn,id_RPf,id_RPs
      ...
      real(rk),allocatable :: sRPR1(:),sRPn5(:)
      real(rk) :: frB1R3,siref
...
!
! !INPUT PARAMETERS:
      class (type_ersem_bacteria_docdyn_sirem),intent(inout),target :: self
...
!BOC
      ...
      call self%get_parameter(self%siref,   'siref',   'mol Si mol C-1','reference Si to C ratio')
      ...
      do iRP=1,self%nRP
         ...
         call self%register_state_dependency(self%id_RPs(iRP),'RP'//trim(index)//'s','mmol Si/m^3', 'silica in substrate '//trim(index))    
         ...
         call self%request_coupling_to_model(self%id_RPs(iRP),self%id_RP(iRP),standard_variables%total_silicate)
         if (use_iron) then
      ...
      allocate(self%sRPn5(self%nRP))
      do iRP=1,self%nRP
         write (index,'(i0)') iRP
         call self%get_parameter(self%sRPR1(iRP),'sRP'//trim(index)//'R1','1/d','remineralisation of substrate '//trim(index)//' to DOM')
         call self%get_parameter(self%sRPn5(iRP),'sRP'//trim(index)//'n5','1/d','remineralisation of substrate '//trim(index)//' to N5')
      end do
   ...
   subroutine do(self,_ARGUMENTS_DO_)
      ...
      class (type_ersem_bacteria_docdyn_sirem),intent(in) :: self
...
!..Source equations
   ...
   subroutine remineralization(self,_ARGUMENTS_DO_)
      ...
      real(rk) :: RPcP(self%nRP),RPsP(self%nRP),fRPN5s(self%nRP)
      real(rk) :: etarr,ETW,tk
...
! Si dissolution
            do iRP=1,self%nRP
               _GET_(self%id_RPc(iRP),RPcP(iRP))
               _GET_(self%id_RPs(iRP),RPsP(iRP))
            end do

           _GET_(self%id_ETW,ETW)

! Arrhenius equation for temperature dependency. 50000 is the the activation
! energy for opal dissolution in J mol-1 (Sarmiento and Gruber 2006) and 8.3 is the universal gas constant
! (J mol -1 K-1). tk is the absolute temperature (K). The reference tmperature
! is assumed to be 10 degree (Luca, 07/2015)

            tk=ETW+273.
            etarr=exp((50000./8.3)*((1./283.)-(1./tk)))

! Si dissolution is assumed to be dependent on Si:C in particles and temperature.

             fRPN5s=(min(10._rk,(RPsP/RPcP)/self%siref)*etarr)*self%sRPn5*RPsP

            do iRP=1,self%nRP
               _SET_ODE_(self%id_RPs(iRP),-fRPN5s(iRP))
            end do

            _SET_ODE_(self%id_N5s,+sum(fRPN5s))
