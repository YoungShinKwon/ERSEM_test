...
module ersem_primary_producer
   ...
   type,extends(type_ersem_pelagic_base),public :: type_ersem_primary_producer
      ...
      !real(rk) :: chn,chp,chf     !!YSK added for the Amundsen Sea
...
!BOC
      ...
      !call self%get_parameter(self%chn,'chn', 'mmol/m^3',  'Michaelis-Menten constant for nitrate limitation')    !!YSK added
      !call self%get_parameter(self%chp,'chp', 'mmol/m^3',  'Michaelis-Menten constant for phosphate limitation')  !!YSK added
      if (self%use_Si) then
      ...
      !   call self%get_parameter(self%chf,'chf', 'umol/m^3',  'Michaelis-Menten constant for iron limitation')  !!YSK added
      end if
   ...
   ! !LOCAL VARIABLES:
      ...
      _LOOP_BEGIN_
         ...
         !instead above,     (by YSK for the Amundsen Sea)
         !iNp = MIN(1.0_rk, N1pP/(N1pP+self%chp))
         !iNn = MIN(1.0_rk, N3nP/(N3nP+self%chn))
         ...
         if (use_iron) then
            ...
            !instead of above,
	    !iNf = MIN(1.0_rk, N7fP/(N7fP+self%chf))
         else
            ! No iron limitation
            iNf = 1.0_rk
         end if
         ...
         !sum = self%sum*et*iNs*min(iNf,iNI)   !YSK modified for the Amundsen Sea model
         ...
         if (self%use_Si) then
            ...
            _SET_ODE_(self%id_RPs, 0.1*fPIRPs)    !!YSK added !_SET_ODE_(self%id_RPs, fPIRPs)
            _SET_ODE_(self%id_N5s, 0.9*fPIRPs)    !!YSK added for silicate balance
   ...
   function get_sinking_rate(self,_ARGUMENTS_LOCAL_) result(SD)
      ...
      real(rk) :: qpc,qnc
      real(rk) :: SD
      real(rk) :: N5s,N1pP,N3nP,N4nP    !!YSK added
      real(rk) :: iNp,iNn,iNI
